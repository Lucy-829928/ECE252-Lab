...
void *do_work(void *arg)
{
    while (1)
    {
        ...
        while (frontier_stack->num_items == 0 && !exit_flag)
        {
            ...
            // Set exit flag is all threads are waiting
            if (sleeping_threads == t)
            {
                ...
                pthread_mutex_unlock(&exit_mutex);
                pthread_mutex_unlock(&frontier_mutex);
                return NULL; // Exit current thread
            }

            pthread_mutex_unlock(&exit_mutex);
            pthread_cond_wait(&frontier_cond, &frontier_mutex); // Wait for work or exit signal
            pthread_mutex_lock(&exit_mutex);
        ...
        // Pop new url from frontier stack
        struct UrlStackElement url;
        pop(frontier_stack, &url);
        pthread_mutex_unlock(&frontier_mutex);

        // Execute curl handling
        CURL *curl_handle;
        CURLcode res;
        RECV_BUF recv_buf;
        ...
        // printf("respond code = %ld, url = %s\n", response_code, url.url_ptr);

        if (res != CURLE_OK)
        {
            // fprintf(stderr, "cURL error for URL %s: %s\n", url.url_ptr, curl_easy_strerror(res));
        }
        ...
        else if (response_code >= 300 && response_code < 400 && final_url && res == CURLE_OK)
        {
            ...
            if (hsearch(entry_original, FIND) == NULL)
            {
                ...
                // Save key to linked list
                KeyNode *new_node = malloc(sizeof(KeyNode));
                new_node->key = entry_original.key; // key address
                new_node->next = key_list;
                key_list = new_node; // update list head
            ...
            if (hsearch(entry_redirected, FIND) == NULL)
            {
                ...
                // Save key to linked list
                KeyNode *new_node = malloc(sizeof(KeyNode));
                new_node->key = entry_redirected.key; // key address
                new_node->next = key_list;
                key_list = new_node; // update list head
        ...
        // EXIT CONDITION 2: total png reach the specified number
        pthread_mutex_lock(&png_mutex);
        if (total_png >= m)
        {
            pthread_mutex_lock(&exit_mutex);
            exit_flag = 1; // Set exit flag
            pthread_mutex_unlock(&exit_mutex);
            pthread_cond_broadcast(&frontier_cond); // Wake all threads
            pthread_mutex_unlock(&png_mutex);
            free(url.url_ptr);
